...
export default function MainPage({ navigation, route }) {
  ...
  const AdmobID = 'ca-app-pub-7815580729420007/1742302091'
  ...
  const getUserData = (username) => {
    firebase_db.ref("data/").once("value").then((snapshot)=>{
      // let nameList = Object.keys(snapshot.val())
      let nameList = ['a', 'woojin', 'b']
      username in nameList ? 
      console.log('있음'):
      console.log('없음')
      firebase_db.ref().child("data").set(username)
      console.log('추가완료')
